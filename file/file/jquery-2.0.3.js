 ...
 */
(function( window, undefined ) {//传入undefined是为了防止undefined被修改，在IE7,8中undefined是可以被修改的。
...
var
	...
	_jQuery = window.jQuery,//和防止冲突有关

	// Map over the $ in case of overwrite
	_$ = window.$,//和防止冲突有关

	// [[Class]] -> type pairs
	class2type = {},//存放类型 例如：class2tyep={"Object String":"String"}

	// List of deleted data cache ids, so we can reuse them
	core_deletedIds = [],//在以前版本是存放数据的缓存，后面版本就没有太大的用处了
	...
	// The ready event handler and self cleanup method  DOM加载完的回调函数
	completed = function() {
		document.removeEventListener( "DOMContentLoaded", completed, false );
		window.removeEventListener( "load", completed, false );
		jQuery.ready();
	};
//99-283给jQuery添加属性和方法
jQuery.fn = jQuery.prototype = {
	//---- The current version of jQuery being used
	jquery: core_version,//版本
//-----进行修改，防止构造函数被修改，例如 function A(){} A.prototype.name="11"; A.prototype.age=20;  var  a  =new A(); 这样的a.constructor=function A(){} 
	constructor: jQuery,//---但是如果是这样的 A.prototype={name:"11",age:30} a.constructed=function Object(){};所以这样修改
	init: function( selector, context, rootjQuery ) {//---初始化和参数管理
		...
		//------选择器的判断String   比如$("#div") $(".div") $("div") $("#div div.div") $("<li>") $("<li>1</li><li>2</li>") $("<li>hello")
		// Handle HTML strings
		if ( typeof selector === "string" ) {
			if ( selector.charAt(0) === "<" && selector.charAt( selector.length - 1 ) === ">" && selector.length >= 3 ) {
				//当是$('<li>')或者$('<li>1</li><li>2</li>')这种情况的时候 match=[null,'<li>',null];
				match = [ null, selector, null ];
			} else {
				//当是$("#div") $(".div") $("div") $("#div div.div") $("<li>hello")这样的情况的时候：
				//①$(".div") $("div") $("#div div.div") match=null；
		...
		//makeArray把类数组转换真正数组（类数组）一个参数是转化为数组，传入两个参数，就是转换为json
	...
	get: function( num ) {//转原生对象，在是用$("....").get()的时候根据不同的参数就会有不同结果
		return num == null ?
	...
	each: function( callback, args ) {//加强版的for循环
	...
	end: function() {//回溯到栈的下一层，没有的话就走一个空
